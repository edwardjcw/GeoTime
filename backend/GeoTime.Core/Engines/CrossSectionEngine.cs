...
public sealed class CrossSectionEngine(int sampleCount = CrossSectionEngine.DEFAULT_SAMPLE_COUNT)
{
    ...
    public static List<DeepEarthZone> GetDeepEarthZones() =>
    [
    ...
    {
        var d = CentralAngle(lat1, lon1, lat2, lon2);
    ...
    public CrossSectionProfile? BuildProfile(List<LatLon> pathPoints)
    {
        ...
        var gs = _state.GridSize;
        var samples = SamplePathPoints(pathPoints, sampleCount);
        var totalDist = ComputePathDistanceKm(pathPoints);

        var result = new List<CrossSectionSample>();
        for (var s = 0; s < samples.Count; s++)
        {
            var distKm = sampleCount > 1 ? (double)s / (sampleCount - 1) * totalDist : 0;
            var ci = LatLonToIndex(samples[s].Lat, samples[s].Lon, gs);
    ...
    public static List<LatLon> SamplePathPoints(List<LatLon> pts, int numSamples)
    {
        ...
        for (var i = 0; i < segs.Length; i++) cum[i + 1] = cum[i] + segs[i];

        var result = new List<LatLon>();
        for (var s = 0; s < numSamples; s++)
        {
            var frac = numSamples > 1 ? (double)s / (numSamples - 1) : 0;
            var target = frac * totalAngle;
            var seg = 0;
            while (seg < segs.Length - 1 && cum[seg + 1] < target) seg++;
            var t = segs[seg] > 1e-12 ? (target - cum[seg]) / segs[seg] : 0;
